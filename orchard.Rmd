...
This figure above conveys the information correctly, but it is erroneous in the way it examines the number of samples.
...
## when n=1 samples,
for (n in 1:100){
nn <- do.call(rbind, lapply(1:n.sim, function(i){deterministic.list[[i]][n,"estimate"]}))
tt <- table(nn > sum(twotier.orchard.df$n.apples))
print(paste0("n=", n, " p(overestimate) = ", tt[2]/sum(tt)))
}

```

Why is this the case that our number of samples doesn't improve our chance of not overestimating the number of apples in the orchard? And why is it not 50% across the board? Why are the probabilities of overestimating the number of apples smaller when we have an odd number of samples?
